...
function [labels, map] = assignSemanticsNew3(centroids, current_labels),
	...
	order = arg_sort(centroids, 1);	% Sort based on Feature 2
	polls(order(1), 1) = hard_label;	% Lowest value is hard
	polls(order(2), 1) = medium_label;	% Medium value
	polls(order(3), 1) = easy_label; % Highest value is easy
	...
	order = arg_sort(centroids, 2);	% Sort based on Feature 4
	polls(order(1), 2) = easy_label;	% Lowest value is easy
	polls(order(2), 2) = medium_label;	% Medium value
	polls(order(3), 2) = hard_label; % Highest value is hard
	...
	% order = arg_sort(centroids, 3);	% Sort based on Feature 5
	% polls(order(1), 3) = easy_label;	% Lowest value is easy
	% polls(order(2), 3) = medium_label;	% Medium value
	% polls(order(3), 3) = hard_label; % Highest value is hard
	...
	% order = arg_sort(centroids, 4);	% Sort based on Feature 6
	...
	order = arg_sort(centroids, 3);	% Sort based on Feature 7
	polls(order(1), 3) = hard_label;	% Lowest value is hard
	polls(order(2), 3) = medium_label;	% Medium value
	polls(order(3), 3) = easy_label; % Highest value is easy
