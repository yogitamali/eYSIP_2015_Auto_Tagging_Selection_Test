...
% Main file which drives SVM to deduce a model to predict
...
% This file uses the libsvm library for training and predicting the 
% output for SVM. See Copyright
...
% load('comparisondata/trainingX');
% load('comparisondata/trainingY');
% load('comparisondata/testX');
% load('comparisondata/testY');
...
fprintf("\n\nTraining SVM...\n");
fflush(stdout);
model = svmtrain(trainingY, trainingX);
...
fprintf("\n\nEstimating Errors...\n");
fflush(stdout);
[output, accuracy, dummy] = svmpredict(testY, testX, model);
